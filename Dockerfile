...
RUN git clone -b v1.8 https://github.com/camenduru/text-generation-webui && \
...
CMD cd text-generation-webui && python3 server.py --listen --chat --load-in-8bit --model /content/text-generation-webui/models/Llama-2-13b-chat-hf
